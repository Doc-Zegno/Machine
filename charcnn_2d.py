...
class CharCNN(nn.Module):
    def __init__(self, char_size, output_size, kernel_width=3):
        ...
        self.conv = nn.Conv2d(1, output_size, (char_size, kernel_width))
    ...
    def forward(self, form):
        ...
        # Conv2d expects:
        # BATCH_SIZE x INPUT_CHANNELS x H x W
        # i.e.:
        # 1 x 1 x CHAR_SIZE x WORD_LEN
        embeddings = embeddings.view((-1, self.char_size))
        embeddings = embeddings.t()
        embeddings = embeddings.contiguous()
        embeddings = embeddings.view((1, 1, self.char_size, -1))
        # embeddings = self.reflection(embeddings)
        features = self.conv(embeddings)

        # Features are 1 x OUTPUT_SIZE x 1 x WORD_LEN (-kernel_width...)
